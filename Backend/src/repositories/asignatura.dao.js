...
export const getAsignaturas = async () => {
  ...
  export const getAsignaturaById = async (cod_asignatura) => {
    try {
      const result = await pool.query("SELECT * FROM asignatura WHERE cod_asignatura = $1", [cod_asignatura]);
  ...
  export const createAsignatura = async (cod_asignatura, nombre_asignatura, cod_pregrado, cod_curso, horas_semanales, sede) => {
    try {
  ...
  export const updateAsignatura = async (cod_asignatura, nombre_asignatura, cod_pregrado, cod_curso, horas_semanales, sede) => {
    try {
      const result = await pool.query(
  ...
  export const deleteAsignatura = async (cod_asignatura) => {
    try {
      const result = await pool.query("DELETE FROM asignatura WHERE cod_asignatura = $1 RETURNING *", [cod_asignatura]);
      return result.rows[0];
    } catch (error) {
